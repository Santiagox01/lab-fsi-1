...
import { PasajeroDeleteDialog } from '../delete/pasajero-delete-dialog';
...
export class Pasajero {
  readonly pasajeros = signal<IPasajero[]>([]);
  ...
  readonly isLoading = this.pasajeroService.pasajerosResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.pasajeroService.pasajerosResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.pasajeros.set(this.fillComponentAttributesFromResponseBody([...this.pasajeroService.pasajeros()]));
    });
    effect(() => {
  ...
  trackId = (item: IPasajero): number => this.pasajeroService.getPasajeroIdentifier(item);

  delete(pasajero: IPasajero): void {
    const modalRef = this.modalService.open(PasajeroDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IPasajero[]): IPasajero[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.pasajeroService.pasajerosParams.set(queryObject);
  }
