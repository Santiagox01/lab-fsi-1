...
import { sampleWithRequiredData } from '../pasajero.test-samples';
...
describe('Pasajero Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 4758 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.pasajeros()[0]).toEqual(expect.objectContaining({ id: 4758 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 4758 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.pasajeros()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 4758 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to pasajeroService', () => {
      ...
      vi.spyOn(service, 'getPasajeroIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPasajeroIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.pasajerosParams()).toMatchObject({ sort: ['id,desc'] });
  });
