...
import { sampleWithRequiredData } from '../vuelo.test-samples';
...
describe('Vuelo Management Component', () => {
  ...
  let comp: Vuelo;
  let fixture: ComponentFixture<Vuelo>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Vuelo);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 469 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.vuelos()[0]).toEqual(expect.objectContaining({ id: 469 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 469 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.vuelos()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 469 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to vueloService', () => {
      const entity = { id: 469 };
      vi.spyOn(service, 'getVueloIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.vuelosParams()).toMatchObject({ sort: ['id,desc'] });
  });
