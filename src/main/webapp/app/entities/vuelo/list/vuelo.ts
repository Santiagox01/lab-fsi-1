...
import { VueloDeleteDialog } from '../delete/vuelo-delete-dialog';
import { VueloService } from '../service/vuelo.service';
import { IVuelo } from '../vuelo.model';

@Component({
  selector: 'jhi-vuelo',
  templateUrl: './vuelo.html',
  imports: [
    RouterLink,
    FontAwesomeModule,
    AlertError,
...
export class Vuelo {
  ...
  readonly isLoading = this.vueloService.vuelosResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.vueloService.vuelosResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.vuelos.set(this.fillComponentAttributesFromResponseBody([...this.vueloService.vuelos()]));
    });
    effect(() => {
  ...
  trackId = (item: IVuelo): number => this.vueloService.getVueloIdentifier(item);

  delete(vuelo: IVuelo): void {
    const modalRef = this.modalService.open(VueloDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IVuelo[]): IVuelo[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.vueloService.vuelosParams.set(queryObject);
  }
