...
import { ReservaDeleteDialog } from '../delete/reserva-delete-dialog';
...
export class Reserva {
  readonly reservas = signal<IReserva[]>([]);
  ...
  readonly isLoading = this.reservaService.reservasResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.reservaService.reservasResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.reservas.set(this.fillComponentAttributesFromResponseBody([...this.reservaService.reservas()]));
    });
    effect(() => {
  ...
  delete(reserva: IReserva): void {
    const modalRef = this.modalService.open(ReservaDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IReserva[]): IReserva[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.reservaService.reservasParams.set(queryObject);
  }
