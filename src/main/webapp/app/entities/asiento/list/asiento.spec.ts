...
describe('Asiento Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 20063 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 20063 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.asientos()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 20063 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to asientoService', () => {
      ...
      vi.spyOn(service, 'getAsientoIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.asientosParams()).toMatchObject({ sort: ['id,desc'] });
  });
