...
import { AsientoDeleteDialog } from '../delete/asiento-delete-dialog';
...
export class Asiento {
  readonly asientos = signal<IAsiento[]>([]);
  ...
  readonly isLoading = this.asientoService.asientosResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.asientoService.asientosResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.asientos.set(this.fillComponentAttributesFromResponseBody([...this.asientoService.asientos()]));
    });
    effect(() => {
  ...
  delete(asiento: IAsiento): void {
    const modalRef = this.modalService.open(AsientoDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IAsiento[]): IAsiento[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.asientoService.asientosParams.set(queryObject);
  }
